    ...
    for col = colmat
        for site = sitemat
            ...
            if ~exist([datapath,'cellinfo_',shot,'.mat'],'file')
                ...
                if exist([datapath,'tracedata_withPuncta_',shot,'.mat'],'file')
                    
                    cells = cellinfo_V2foci(datapath, row, col, site, nuc_col, cyto_col, framerate);
                    load([datapath,'tracedata_withPuncta_',shot,'.mat'],'tracedata');
                else
                    continue;
                end
            else
