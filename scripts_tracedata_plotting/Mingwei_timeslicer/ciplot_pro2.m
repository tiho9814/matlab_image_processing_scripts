...
%20180112 Andrew edits added as lines 19-30 so that the code doesn't crash
%when there is only a single trace and mean cannot be calculated. 
if size(input,2)==1 && size(input,1)>1
    disp ('looks like a column vector, need rows');
end
only_one=size(input,1)==1;

if only_one
      %%if there is only one trace
    %% making the plot
    line(x,input,'color',colour);
    hold on

else
    %% this works when there is more than one trace
